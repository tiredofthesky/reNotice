...
class IosSelector {
    ...
    `;
		...
		if (this.type === 'infinite') {
			this.elems.highlightList.style.top = -50 + 'px';
			this.elems.highlightList.style.left = -20 + 'px';
		}
	...
	_moveTo(scroll) {
		...
		// console.log(this.itemAngle, scroll, this.itemAngle * scroll);
		this.elems.circleList.style.transform = (this.itemAngle * scroll) < 0 ? 0 : `translate3d(0, 0, ${-this.radius}px) rotateX(${this.itemAngle * scroll}deg)`;
...
// // time
export let hours = new Array(24).fill(1).map((v, i) => {
	return { value: i + 1, text: i + 1 }
});
export let minutes = new Array(60).fill(1).map((v, i) => {
	return { value: i + 1, text: i + 1 }
});
